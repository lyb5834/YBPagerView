...
@interface YBPagerListContainerScrollView: UIScrollView <UIGestureRecognizerDelegate>
@property (nonatomic, assign, getter=isCategoryNestPagingEnabled) BOOL categoryNestPagingEnabled;
@end
@implementation YBPagerListContainerScrollView
...
@interface YBPagerListContainerCollectionView: UICollectionView <UIGestureRecognizerDelegate>
@property (nonatomic, assign, getter=isCategoryNestPagingEnabled) BOOL categoryNestPagingEnabled;
@end
@implementation YBPagerListContainerCollectionView
- (BOOL)gestureRecognizerShouldBegin:(UIGestureRecognizer *)gestureRecognizer {
    if (self.isCategoryNestPagingEnabled) {
        if ([gestureRecognizer isMemberOfClass:NSClassFromString(@"UIScrollViewPanGestureRecognizer")]) {
            CGFloat velocityX = [(UIPanGestureRecognizer *)gestureRecognizer velocityInView:gestureRecognizer.view].x;
...
@interface YBPagerListContainerViewController : UIViewController
...
@implementation YBPagerListContainerViewController
- (void)dealloc
{
...
@interface YBPagerListContainerView () <UIScrollViewDelegate, UICollectionViewDelegate, UICollectionViewDataSource>
@property (nonatomic, weak) id<YBPagerListContainerViewDelegate> delegate;
...
@property (nonatomic, strong) YBPagerListContainerViewController *containerVC;
@end

@implementation YBPagerListContainerView

- (instancetype)initWithType:(YBPagerListContainerType)type delegate:(id<YBPagerListContainerViewDelegate>)delegate{
...
- (void)initializeViews {
    ...
    _containerVC = [[YBPagerListContainerViewController alloc] init];
    ...
    if (self.containerType == YBPagerListContainerType_ScrollView) {
        if (self.delegate &&
            ...
            _scrollView = [[YBPagerListContainerScrollView alloc] init];
        }
        ...
        if (self.delegate &&
            ...
            _collectionView = [[YBPagerListContainerCollectionView alloc] initWithFrame:CGRectZero collectionViewLayout:layout];
        }
...
- (void)layoutSubviews {
    ...
    if (self.containerType == YBPagerListContainerType_ScrollView) {
        if (CGRectEqualToRect(self.scrollView.frame, CGRectZero) ||  !CGSizeEqualToSize(self.scrollView.bounds.size, self.bounds.size)) {
            ...
            [_validListDict enumerateKeysAndObjectsUsingBlock:^(NSNumber * _Nonnull index, id<YBPagerViewListViewDelegate>  _Nonnull list, BOOL * _Nonnull stop) {
...
- (void)setCategoryNestPagingEnabled:(BOOL)categoryNestPagingEnabled {
    ...
    if ([self.scrollView isKindOfClass:[YBPagerListContainerScrollView class]]) {
        ((YBPagerListContainerScrollView *)self.scrollView).categoryNestPagingEnabled = categoryNestPagingEnabled;
    }else if ([self.scrollView isKindOfClass:[YBPagerListContainerCollectionView class]]) {
        ((YBPagerListContainerCollectionView *)self.scrollView).categoryNestPagingEnabled = categoryNestPagingEnabled;
    }
}
...
- (UICollectionViewCell *)collectionView:(UICollectionView *)collectionView cellForItemAtIndexPath:(NSIndexPath *)indexPath {
    ...
    id<YBPagerViewListViewDelegate> list = _validListDict[@(indexPath.item)];
    if (list != nil) {
...
#pragma mark - YBCategoryViewListContainer
...
- (void)reloadData {
    for (id<YBPagerViewListViewDelegate> list in _validListDict.allValues) {
...
- (void)initListIfNeededAtIndex:(NSInteger)index {
    ...
    id<YBPagerViewListViewDelegate> list = _validListDict[@(index)];
    if (list != nil) {
        //列表已经创建好了
        return;
    }
