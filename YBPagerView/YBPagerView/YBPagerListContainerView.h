...
@class YBPagerListContainerView;
@class YBPagerListContainerScrollView;

@protocol YBPagerViewListViewDelegate <NSObject>
 ...
 */
typedef NS_ENUM(NSUInteger, YBPagerListContainerType) {
    YBPagerListContainerType_ScrollView,
    YBPagerListContainerType_CollectionView,
};

@protocol YBPagerListContainerViewDelegate <NSObject>
/**
 返回list的数量
...
/**
 根据index返回一个对应列表实例，需要是遵从`YBPagerViewListViewDelegate`协议的对象。
 ...
 @param listContainerView 列表的容器视图
 @param index 目标下标
 @return 遵从YBPagerViewListViewDelegate协议的list实例
 */
...
/**
 ...
 @param listContainerView YBPagerListContainerView
 ...
 */
- (BOOL)listContainerView:(YBPagerListContainerView *)listContainerView canInitListAtIndex:(NSInteger)index;
...
@interface YBPagerListContainerView : UIView

@property (nonatomic, assign, readonly) YBPagerListContainerType containerType;
@property (nonatomic, strong, readonly) UIScrollView *scrollView;
@property (nonatomic, strong, readonly) NSDictionary <NSNumber *, id<YBPagerViewListViewDelegate>> *validListDict;   //已经加载过的列表字典。key是index，value是对应的列表
...
- (instancetype)initWithType:(YBPagerListContainerType)type delegate:(id<YBPagerListContainerViewDelegate>)delegate NS_DESIGNATED_INITIALIZER;

@end

@interface YBPagerListContainerView (ListContainer)
